...
import org.springframework.data.cassandra.examples.core.model.VisitorProfileChangeLog;
import org.springframework.data.cassandra.examples.core.model.VisitorProfileChangeLogPK;
...
public class CassandraTemplateExample {
	...
	public static void main(String[] args) throws UnknownHostException {
		...
		try {
			...
			CassandraOperations template2 = new CassandraTemplate(connect(HOSTNAME, "digital_checkout"));

			 VisitorProfileChangeLogPK key = new VisitorProfileChangeLogPK("user1", new Date());
			 VisitorProfileChangeLog changeLog= new VisitorProfileChangeLog();
			 
			 changeLog.setKey(key);
			 
			 VisitorProfileChangeLog result = template2.insert(changeLog);

				LOGGER.info("VisitorProfileChangeLog [{}]", result);
	...
	protected static synchronized Session connect(String hostname, int port, String keyspace) {
		if (cluster == null) {
			...
			cluster = Cluster.builder().addContactPointsWithPorts(Collections.singleton(
				NetworkUtils.newSocketAddress(hostname, port))).
